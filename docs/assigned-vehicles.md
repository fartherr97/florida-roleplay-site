...
Both the API and navigation require `development.assignments.view`. Defaults are `developer`, `directorship`, `ownership`; the exact Developer Discord role must map to the `developer` key in the site's role map. Ownership can configure this read-only permission in Access & Roles. It does not grant model approvals or ticket access.
