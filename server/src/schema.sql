...
-- The staff team's own internal ledger — resignations, LOAs, strikes,
-- terminations kept ON staff members. Deliberately separate from
-- disciplinary_actions: nothing here reaches a background check, and only
-- Senior Admins+ can read or write it. A termination sets archived and snapshots
-- the member's info into archive_snapshot, so a former staff member's record
-- still has context after their roster row is cleared.
CREATE TABLE IF NOT EXISTS staff_admin_logs (
  id                    BIGINT GENERATED BY DEFAULT AS IDENTITY,
  type                  VARCHAR(32)  NOT NULL,
  target_name           VARCHAR(128) NOT NULL,
  target_discord_id     VARCHAR(20)  NOT NULL,
...
-- One current email per member, keyed by Discord id. Anyone may set their own
-- (via the bot's /email add); Directorship+ read the directory on the site and
-- look one up in Discord. Gmail-only is enforced where it is written.
...
-- Every address a member has had, so the directory can show what changed. A row
-- is written when an address is replaced, never on the first add.
...
CREATE INDEX IF NOT EXISTS idx_member_email_history ON member_email_history (discord_id, replaced_at DESC);
CREATE INDEX IF NOT EXISTS idx_member_emails_email ON member_emails (LOWER(email));
